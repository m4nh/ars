...
class WInstanceEditor(WBaseWidget):
    ...
    def __init__(self,changeCallback=None):
        ...
        self.ui_axis_translation = WAxesEditor(names=['cx','cy','cz'],labels=['X','Y','Z'],unit='m',step=0.001,changeCallback=self.valueChanged)
        self.ui_axis_rotation = WAxesEditor(names=['roll','pitch','yaw'],labels=['R','P','Y'],unit='°',step=0.2,binding_conversion=180.0/math.pi, changeCallback=self.valueChanged)
        self.ui_axes_container.addWidget(self.ui_axis_translation)
        self.ui_axes_container.addWidget(self.ui_axis_rotation)

        self.ui_axes_relative_buttons = WAxesButtons(name='relative', label='Relative Movement', changeCallback=self.axesButtonCallback)
        self.ui_axes_size_butttons = WAxesButtons(name='size',label='Size', changeCallback=self.axesButtonCallback)
        
        self.ui_relative_axes_container.addWidget(self.ui_axes_relative_buttons)
    ...
    def classListChange(self, index):
        ...
        combo.setStyleSheet("QComboBox::drop-down {background: " + color + ";}")
        if self.instance!=None:
