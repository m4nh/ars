...
def visualize(image_file, correct_detection, mistakes, missed, gt_boxes):
    ...
    immy_correct = cv_show_detection.draw_prediction(
        immy, correct_detection, color_map=cmap, min_score_th=0.01)
    immy_mistake = cv_show_detection.draw_prediction(
        immy_copy, mistakes, color_map=cmap, min_score_th=0.01)
    immy_gt = cv_show_detection.draw_prediction(
        immy_copy_2, gt_boxes, color_map=cmap, min_score_th=0)
    immy_missed = cv_show_detection.draw_prediction(
        immy_copy_3, missed, color_map=cmap, min_score_th=0.01)
...
def make_table(to_write, scores):
    format_string = '{};{};{};{};{};{};{}\n'
    to_write.append(
        'CONF_TH;TP;Predictions;Precision;Recall;F-Score;AVG_IOU;\n')
    average_precision = 0
    global_avg_IOU = 0
    last_recall = 0
    precisions = []
...
if __name__=='__main__':
    ...
    if args.verbosity > 1 and args.image is None:
        print(
            'Verbosity set to max, please specify the folder were the image will be loaded with -i ${PATH}')
        exit()
    ...
    to_check = [args.predicted, args.label, args.image] if args.verbosity > 1 else [
        args.predicted, args.label]
    for f in to_check:
    ...
    for img_indx,(l,p) in enumerate(zip(labels,predicted)):
        ...
        missed = [box for m, box in zip(
            gt_map, gt_boxes) if m['index'] is None]
        if args.out_folder is not None:
            out_file = os.path.join(
                args.out_folder, os.path.basename(l).replace('.txt', '.json'))
            with open(out_file, 'w+') as f_out:
                json.dump({'correct': correct_detection, 'mistakes': mistakes,
                           'missed': missed}, f_out, default=lambda o: o.__dict__)

        if args.verbosity > 1:
            image_file = os.path.join(args.image, os.path.basename(l))
            visualize(image_file, correct_detection,
                      mistakes, missed, gt_boxes)
        ...
        for k in class_scores:
            for i, c in enumerate(CONFIDENCE_TH):
                class_scores[k][i]['Detected'] += get_detected_for_th(
                    gt_map, c, k)
    ...
    plot(precisions, recals, show=args.verbosity >
